...
"""
        ...
        assert "document_type" in result.warnings[0] if result.warnings else True
...
"""
        ...
        assert any("No YAML frontmatter found" in err for err in result.errors)
...
"""
        result = validate_yaml_frontmatter(content)
        assert not result.is_valid
        assert any("Failed to parse YAML" in err for err in result.errors)
...
"""
        result = validate_yaml_frontmatter(content)
        assert not result.is_valid
        assert any("must be an integer" in err for err in result.errors)
...
"""
        result = validate_traceability(content)
        assert any("No requirement IDs found" in warn for warn in result.warnings)
...
phase: 1
audience: [architects]
traceability: [src-v02/03_eva_chat_requirements.md]
---
...
"""
        result = validate_traceability(content)
        assert not result.is_valid
        assert any("src-v02/" in err for err in result.errors)
...
Reference to src-v02/03_eva_chat_requirements.md.
"""
        result = validate_traceability(content)
        assert "INF01" in result.warnings[0] if result.warnings else True
        assert any("src-v02/" in warn for warn in result.warnings)
...
Requirement INF01 (src-v02/03_eva_chat_requirements.md#L5-L8) is satisfied.
...
No requirement IDs or src-v02 references.
"""
...
def test_end_to_end_validation(sample_valid_content):
    ...
    assert cons_result.is_valid
    assert trace_result.is_valid
...
def test_end_to_end_validation_failure(sample_invalid_content):
    ...
    assert not yaml_result.is_valid
    assert not cons_result.is_valid  # 'see above' reference
    assert not trace_result.is_valid  # No src-v02 reference
