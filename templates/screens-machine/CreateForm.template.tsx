/**
 * Auto-generated CreateForm for {{LAYER_NAME}} layer.
 *
...
export interface {{LAYER_NAME}}CreateFormProps {
  /** Called with serialised form data on successful submission */
...
export const {{LAYER_NAME}}CreateForm: React.FC<{{LAYER_NAME}}CreateFormProps> = ({
  onSubmit,
  onCancel,
  ...
  useEffect(() => {
    ...
    const fetchSchema = async () => {
      try {
        ...
        console.warn(
          '[{{LAYER_NAME}}CreateForm] Schema fetch failed – using offline snapshot:',
          err,
        );
  ...
  // -------------------------------------------------------------------------
  // Initialise form values whenever the schema changes.
  // -------------------------------------------------------------------------
  useEffect(() => {
    const defaults: FormData = {};
    schema.fields.forEach(field => {
      defaults[field.field_name] = field.field_type === 'boolean' ? false : '';
    });
    ...
    const loadReferenceOptions = async () => {
      ...
      for (const field of refFields) {
        ...
        try {
          ...
          console.warn(
            `[{{LAYER_NAME}}CreateForm] Could not load options for "${field.field_name}":`,
            err,
          );
  ...
  const renderField = (field: FieldSchema): React.ReactElement | null => {
    if (!field.field_name) {
      console.warn('[{{LAYER_NAME}}CreateForm] Skipping field with missing field_name:', field);
      return null;
    }
    ...
    switch (field.field_type) {
      ...
      default:
        console.warn(
          `[{{LAYER_NAME}}CreateForm] Unknown field type "${field.field_type}" for field ` +
  ...
  return (
    <form
      ...
      aria-label={t('form.createTitle', { layer: '{{LAYER_NAME}}' })}
      noValidate
    >
      {usingOfflineSchema && (
      ...
      <div className="form-actions">
        ...
        <button type="submit" disabled={isSubmitting}>
          {isSubmitting ? t('form.submitting') : t('form.create', { layer: '{{LAYER_NAME}}' })}
        </button>
      </div>
    </form>
  );
...
export default {{LAYER_NAME}}CreateForm;
