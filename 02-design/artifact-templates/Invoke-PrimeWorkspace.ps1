<#
.SYNOPSIS
    Invoke-PrimeWorkspace.ps1 v1.0.0
    Idempotent project primer: copilot-instructions + PLAN + STATUS + ACCEPTANCE + README
...
.DESCRIPTION
    ...
    Idempotency rules:
      copilot-instructions.md  -- Apply script backs up + preserves PART 2 (always safe)
...
.PARAMETER WorkspaceRoot
    ...
    (e.g. C:\AICOE\eva-foundation). Primes every sub-folder that matches
    the pattern [0-9][0-9]-*.
...
.PARAMETER DataModelBase
    Base URL for the EVA data model API. Default: http://localhost:8010

.PARAMETER Actor
    Value placed in X-Actor header on data model writes. Default: agent:copilot

.EXAMPLE
    # Single project, dry-run preview
    .\Invoke-PrimeWorkspace.ps1 -TargetPath "C:\AICOE\eva-foundation\01-documentation-generator" -DryRun

.EXAMPLE
    # Single project, apply for real
    .\Invoke-PrimeWorkspace.ps1 -TargetPath "C:\AICOE\eva-foundation\01-documentation-generator"

.EXAMPLE
    # Prime every project under the workspace
    .\Invoke-PrimeWorkspace.ps1 -WorkspaceRoot "C:\AICOE\eva-foundation"

.NOTES
    Encoding: ASCII-only. No emoji. Tokens: [PASS] [FAIL] [WARN] [INFO] [SKIP] [DRY]
    Version: 1.0.0
...
param(
    ...
    [string]$DataModelBase = "http://localhost:8010",
...
$Templates = @{
    ...
    Acceptance = Join-Path $GovDir "ACCEPTANCE-template.md"
    ReadmeHdr  = Join-Path $GovDir "README-header-block.md"
}
...
function Write-FromTemplate {
    ...
    if ($IsDryRun) {
        ...
        [System.IO.File]::WriteAllText($DestPath, $out, [System.Text.Encoding]::ASCII)
...
function Inject-ReadmeHeader {
    ...
    if ($IsDryRun) {
        ...
        [System.IO.File]::WriteAllText($ReadmePath, $merged, [System.Text.Encoding]::ASCII)
...
function Write-EvidenceRecord {
    ...
    $record | ConvertTo-Json -Depth 5 | Out-File $evPath -Encoding ASCII
...
function Invoke-PrimeProject {
    ...
    if (-not $SkipCopilotInstructions) {
        Write-Status "[INFO]" "Step 1: copilot-instructions (Apply-Project07-Artifacts.ps1)"
        $applyArgs = @("-TargetPath", $ProjectPath, "-NonInteractive")
        if ($IsDryRun) { $applyArgs += "-DryRun" }
        try {
            & powershell.exe -NonInteractive -ExecutionPolicy Bypass -File $ApplyScript @applyArgs | Out-Null
            Write-Status "[PASS]" "copilot-instructions applied"
            $summary.steps += "copilot-instructions:PASS"
        } catch {
            Write-Status "[WARN]" "Apply script error: $_"
            $summary.steps += "copilot-instructions:WARN"
        }
    } else {
    ...
    if (Test-Sentinel -FilePath $planPath -Sentinel "<!-- eva-primed-plan -->") {
        ...
        $inject   = "<!-- eva-primed-plan -->`n`n## EVA Ecosystem Tools`n`n- Data model: GET $DataModelBase/model/projects/$folderName`n- 29-foundry agents: C:\AICOE\eva-foundation\29-foundry\agents\`n- 48-eva-veritas audit: run audit_repo MCP tool`n`n---`n`n"
        if ($IsDryRun) {
            Write-Status "[DRY]" "Would inject eva-primed-plan block into existing PLAN.md"
        } else {
            [System.IO.File]::WriteAllText($planPath, $inject + $existing, [System.Text.Encoding]::ASCII)
    ...
    if (Test-Sentinel -FilePath $statusPath -Sentinel "<!-- eva-primed-status -->") {
        ...
        $newEntry  = "`n`n---`n`n## $PrimeDate -- Re-primed by $PrimeActor`n`n<!-- eva-primed-status -->`n`nData model: GET $DataModelBase/model/projects/$folderName`n29-foundry agents: C:\AICOE\eva-foundation\29-foundry\agents\`n48-eva-veritas: run audit_repo MCP tool`n"
        if ($IsDryRun) {
            Write-Status "[DRY]" "Would append eva-primed-status session entry to STATUS.md"
        } else {
            [System.IO.File]::WriteAllText($statusPath, $existing + $newEntry, [System.Text.Encoding]::ASCII)
...
if ($TargetPath) {
    ...
    foreach ($proj in $projects) {
        $allResults += Invoke-PrimeProject -ProjectPath $proj.FullName -IsDryRun $DryRun.IsPresent
    }
} else {
...
# Summary
Write-Information ""
Write-Information "=== Invoke-PrimeWorkspace Summary ==="
