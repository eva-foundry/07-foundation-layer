# {{PROJECT_LABEL}} -- Acceptance Criteria
...
**Criteria**: `GET /model/projects/{{PROJECT_FOLDER}}` returns maturity, phase, pbi_total >= 1.
**Verification**:
```powershell
...
**Verification**: Run MCP tool `audit_project` with target_path = {{TARGET_PATH}}
**Status**: PENDING
...
**Verification**: Run MCP tool `get_trust_score` with repo_path = {{TARGET_PATH}}
**Status**: PENDING
...
**Verification**: Run MCP tool `check_encoding` with target_path = {{TARGET_PATH}}
**Status**: PENDING
